...
const Result = (props) => {
  ...
  return (
    <div className="ml-6 min-w-[25%] max-w-[25%] bg-white rounded-lg relative flex flex-col items-center overflow-hidden">
      <span className="text-2xl font-bold mt-8 truncate" style={{ color: getMajorityColor(majorityRating.name) }}>Rating: {majorityRating.name}</span>
      ...
      <div className="flex justify-center w-full px-2 flex-wrap">
        {relevantKeys.map((key) => (
          <div key={key} className="flex items-center mr-4 text-xl font-semibold" style={{ color: getMajorityColor(key), flexShrink: 0 }}>
            <div className="w-4 h-4 mr-2" style={{ backgroundColor: getMajorityColor(key), borderRadius: '2px' }}></div>
            <span className="min-w-0 truncate">{ratings[key.toLowerCase()]}</span>
            <span className="ml-1 whitespace-nowrap">{key}</span>
          </div>
        ))}
      </div>
    </div>
